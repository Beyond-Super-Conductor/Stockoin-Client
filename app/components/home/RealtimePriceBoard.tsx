      ...
      ticker && Object.keys(ticker).map((key) => {
        return (
          <div key={key} className={` w-full flex items-start justify-around border-b border-b-slate-400/60 pb-1`}>
              ...
              <span className='flex-1 text-xl text-center'>{key.replace('KRW-','')}</span>
              <p className={`relative flex-1 text-xl flex items-center justify-center ${ticker[key].isRising ? 'text-red-500' : 'text-blue-400'}`}>
              ...
              <span className={`flex-1 text-xl text-center ${ticker[key].scr > 0 ? 'text-red-500' : 'text-blue-400'}`}> {(ticker[key].scr * 100).toFixed(3)}%</span>
              <span className='flex-1 text-xl text-center'> {(ticker[key].tv).toFixed(3)}개</span>
              
              {
                isExpanded && <>
                  <span className='flex-1 text-xl text-center'>{ticker[key].ab}</span>
                  <span className='flex-1 text-xl text-center'>{ticker[key].c}</span>
                  <span className='flex-1 text-xl text-center'>{format(ticker[key].tms ?? 1, 'MM월 dd일 HH:mm:ss')}</span>
                </>
              }
          </div>
        )
